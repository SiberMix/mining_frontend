...
export const CalendarControl = memo(
  ({
    ...
    const viewOptions = [
      { id: Views.DAY, label: t('День') },
      { id: Views.WEEK, label: t('Неделя') },
      { id: Views.MONTH, label: t('Месяц') },
    ];
    ...
    const dateText = useMemo(() => {
      ...
      if (view === Views.WEEK) {
        ...
        const to = moment(date).locale('ru')?.endOf('week');
    ...
    return (
      <div className='CalendarControl'>
        ...
        <div className='CalendarControl_buttons'>
          <StyledButton onClick={onPrevClick} disabled={isLoading}>
            <ArrowLeftOutlined />
          </StyledButton>
          <div className='CalendarControl_buttons_value'>{dateText}</div>
          <StyledButton onClick={onNextClick} disabled={isLoading}>
            <ArrowRightOutlined />
          </StyledButton>
        </div>
        <Radio.Group
