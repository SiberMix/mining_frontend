...
export const SevoInGroups: React.FC<SevoInGroupsProps> = ({ name, id, sowing4YearsAgo, sowing3YearsAgo, sowing2YearsAgo, actualSowing }) => {
  ...
  useEffect(() => {
    ...
    if (ctx) {
      const allPoints = polygons.flatMap(polygon => polygon.coords)
      ...
      polygons.forEach((polygon) => {
        ...
        const color = fieldType?.color || '#6c86ff'

        ctx.fillStyle = color

        draw(ctx, polygon.coords, boundingRect, canvas)
      })
    }
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = polygons.filter((polygon) => polygon.name.toLowerCase().includes(value.toLowerCase()))
